...
namespace Shared.Services
{
    public class JsonHandler
    {
        ...
        {
            // Use Georgian calendar date for consistency with existing files
            // Format: yyyy-MM-dd (e.g., 2025-01-21)
            var georgianDate = GeorgianDateHelper.GetCurrentGeorgianDate().Replace("/", "-");
            return $"report_{georgianDate}.json";
        }
        ...
        public Dictionary<string, object> GetDefaultReportStructure()
        {
            try
            {
                ...
                {
                    { "date", GeorgianDateHelper.GetCurrentGeorgianDate().Replace("/", "-") },
                    { "employees", employees },
                    { "managers", managers },
                ...
                if (data != null)
                {
                    ...
                    if (empCount == 0 && mgrCount == 0)
                    {
                        ...
                        if (previousData != null)
                        {
                            ...
                            // Also carry forward tasks if they exist
                            var prevTasks = previousData.GetValueOrDefault("tasks", new Dictionary<string, object>()) as Dictionary<string, object>;
                            if (prevTasks != null)
                            {
        ...
        private Dictionary<string, object> CreateReportFromPrevious(Dictionary<string, object> previousData)
        {
            try
            {
                ...
                if (prevTasks != null)
                {
                    newData["tasks"] = prevTasks;
                    _logger.LogInformation("Carried forward tasks from previous day");
                }
                else
                {
                    // Reset tasks for new day only if no previous tasks exist
