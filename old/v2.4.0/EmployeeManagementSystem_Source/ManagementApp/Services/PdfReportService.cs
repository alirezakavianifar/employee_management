...
namespace ManagementApp.Services
{
    public class PdfReportService
    {
        ...
        private Table CreateHeaderTable(string reportTitle, PdfFont titleFont, PdfFont headerFont, PdfFont contentFont, string startDate, string endDate)
        {
            ...
            {
                headerCell.Add(new Paragraph($"From {startDate} to {endDate}")
                    .SetFont(contentFont)
                    .SetFontSize(12)
                    .SetTextAlignment(TextAlignment.CENTER)
                    .SetMarginTop(5)
            ...
            headerCell.Add(new Paragraph($"Generated: {DateTime.Now:yyyy/MM/dd HH:mm}")
                .SetFont(contentFont)
                .SetFontSize(10)
                .SetTextAlignment(TextAlignment.CENTER)
                .SetMarginTop(8)
        ...
        private Table CreateFooterTable(PdfFont smallFont)
        {
            ...
            var footerCell = new Cell()
                .Add(new Paragraph("Report generated by Employee Management System")
                    .SetFont(smallFont)
                    .SetFontSize(9)
                    .SetTextAlignment(TextAlignment.CENTER)
                    .SetFontColor(DarkTextColor))
        ...
        public bool ExportReportToPdf(string reportContent, string filePath, string reportTitle = "Report", string assignedTo = "")
        {
            try
            {
                    ...
                    using (var document = new Document(pdf))
                    {
                        ...
                        if (!string.IsNullOrEmpty(assignedTo))
                        {
                            ...
                            var metadataCell = new Cell()
                                .Add(new Paragraph($"Report assigned to: {assignedTo}")
                                    .SetFont(contentFont)
                                    .SetFontSize(11)
                                    .SetFontColor(DarkTextColor))
                        ...
                        {
                            ("Total employees", reportData.TotalEmployees > 0 ? reportData.TotalEmployees.ToString() : "0"),
                            ("Total absences in period", reportData.TotalAbsences > 0 ? reportData.TotalAbsences.ToString() : "0")
                        };
                        
                        var employeeTable = CreateStatisticsTable("Employee statistics", employeeData, headerFont, contentFont);
                        ...
                        {
                            ("Morning shift (average)", $"{reportData.AverageMorningShift:F1}/{capacity}"),
                            ("Afternoon shift (average)", $"{reportData.AverageAfternoonShift:F1}/{capacity}"),
                            ("Night shift (average)", $"{reportData.AverageNightShift:F1}/{capacity}"),
                            ("Max morning shift", $"{reportData.MaxMorningShift}/{capacity}"),
                            ("Max afternoon shift", $"{reportData.MaxAfternoonShift}/{capacity}"),
                            ("Max night shift", $"{reportData.MaxNightShift}/{capacity}")
                        };
                        
                        var shiftTable = CreateStatisticsTable("Shift statistics (average)", shiftData, headerFont, contentFont);
                        ...
                        var taskData = new List<(string, string)>
                        {
                            ...
                            ("In progress", reportData.InProgressTasks.ToString()),
                            ("Pending", reportData.PendingTasks.ToString())
                        };
                        
                        var taskTable = CreateStatisticsTable("Task statistics (period)", taskData, headerFont, contentFont);
                        ...
                        if (reportData.DailyDetails.Count > 0)
                        {
                            ...
                            var dailyHeaderCell = new Cell()
                                .Add(new Paragraph("Daily details")
                                    .SetFont(headerFont)
                                    .SetFontSize(16)
                                    .SetFontColor(HeaderTextColor)
                                    .SetTextAlignment(TextAlignment.RIGHT))
        ...
        private ReportData ParseReportContent(string content)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Report\s+(.+)");
                    if (match.Success)
                ...
                {
                    var match = Regex.Match(cleanLine, @"Start date:\s*(.+)");
                    if (match.Success)
                        reportData.StartDate = match.Groups[1].Value.Trim();
                }
                else if (cleanLine.Contains("End date:"))
                {
                    var match = Regex.Match(cleanLine, @"End date:\s*(.+)");
                    if (match.Success)
                ...
                {
                    var match = Regex.Match(cleanLine, @"Total\s+employees[^:]*:\s*(\d+)");
                ...
                {
                    var match = Regex.Match(cleanLine, @"Total\s+absences[^:]*:\s*(\d+)");
                    if (match.Success && int.TryParse(match.Groups[1].Value, out int count))
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Morning\s+shift[^:]*:\s*([\d.]+)/(\d+)");
                    if (match.Success)
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Afternoon\s+shift[^:]*:\s*([\d.]+)/(\d+)");
                ...
                {
                    var match = Regex.Match(cleanLine, @"Night\s+shift[^:]*:\s*([\d.]+)/(\d+)");
                    if (match.Success && double.TryParse(match.Groups[1].Value, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out double avg))
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Max\s+morning\s+shift[^:]*:\s*(\d+)/(\d+)");
                ...
                {
                    var match = Regex.Match(cleanLine, @"Max\s+afternoon\s+shift[^:]*:\s*(\d+)/(\d+)");
                    if (match.Success && int.TryParse(match.Groups[1].Value, out int max))
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Max\s+night\s+shift[^:]*:\s*(\d+)/(\d+)");
                    if (match.Success && int.TryParse(match.Groups[1].Value, out int max))
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Total\s+tasks[^:]*:\s*(\d+)");
                    if (match.Success && int.TryParse(match.Groups[1].Value, out int count))
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Completed[^:]*:\s*(\d+)");
                    if (match.Success && int.TryParse(match.Groups[1].Value, out int count))
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"In\s+progress[^:]*:\s*(\d+)");
                    if (match.Success && int.TryParse(match.Groups[1].Value, out int count))
                    {
                ...
                {
                    var match = Regex.Match(cleanLine, @"Pending[^:]*:\s*(\d+)");
                    if (match.Success && int.TryParse(match.Groups[1].Value, out int count))
                    {
        ...
        private string FormatReportContent(string content)
        {
            ...
            // Add a proper header
            formattedLines.AppendLine("Report summary");
            ...
            foreach (var line in lines)
            {
                ...
                if (cleanLine.Contains("Start date:") || cleanLine.Contains("End date:"))
                {
                ...
                else if (cleanLine.Contains("absence") || cleanLine.Contains("absences"))
                {
                    formattedLines.AppendLine($"❌ {cleanLine}");
                }
                else if (cleanLine.Contains("Morning shift") || cleanLine.Contains("Afternoon shift"))
                {
                    formattedLines.AppendLine($"⏰ {cleanLine}");
                }
                else if (cleanLine.Contains("tasks") || cleanLine.Contains("Completed") || cleanLine.Contains("In progress"))
                {
                    formattedLines.AppendLine($"📋 {cleanLine}");
                }
                else if (cleanLine.Contains(":") && cleanLine.Length < 50)
                {
                    // Section headers
                    formattedLines.AppendLine($"\n📊 {cleanLine}");
                }
            ...
            formattedLines.AppendLine("\n" + new string('=', 30));
            formattedLines.AppendLine("Notes:");
            formattedLines.AppendLine("• This report was generated automatically");
            formattedLines.AppendLine("• All data was extracted from the Employee Management System");
            formattedLines.AppendLine("• For more information contact the Human Resources department");
        ...
        {
            return line.StartsWith("📊") || (line.Contains(":") && line.Length < 50);
        }
